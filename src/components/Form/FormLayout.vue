<template>
    <div v-show="formdata.isShow!==false" :class="['bw-form-layout',className]">
         <h3 class="bw-form-layout-title" v-show="formdata.isShowTitle">{{(formdata.i18nLabel?$t(formdata.title):formdata.title)}}</h3>
        <el-row :gutter="formdata.padding||0" >
            <el-col v-for="item in formdata.childrens"  :key="item.id" :span="item.span || 24">
                <FormTab :formdata="item">
...
export default {
    ...
    created(){
      //  console.log('formlayout',this.formConfigs);
    },
    data(){
        return {
...
<style scoped>
    .bw-form-layout-title{font-size: .2rem;padding: .1rem 0;}
</style>